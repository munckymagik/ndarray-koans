...
mod cluster_generation_origin {
    ...
    fn origin_cluster() {
        ...
        let n_features = 2;
        let a = Array::random((n_observations, n_features), StandardNormal);
        ...
        // DM: both return a 1-dimensional array (equiv to Vec)
        assert_eq!(centroid.ndim(), 1);
        assert_eq!(variance.ndim(), 1);
        // DM: as this is 1-dim array only a single integer. > 1 would be a tuple
